# This Makefile is for the Bio::Align::Subset extension to perl.
#
...
#     ABSTRACT_FROM => q[lib/Bio/Align/Subset.pm]
#     AUTHOR => [q[Hector Valverde and Juan Carlos Aledo <hvalverdeuma.es>]]
#     BUILD_REQUIRES => {  }
#     CONFIGURE_REQUIRES => {  }
#     LICENSE => q[perl]
#     NAME => q[Bio::Align::Subset]
#     PL_FILES => {  }
#     PREREQ_PM => { Test::More=>q[0] }
#     VERSION_FROM => q[lib/Bio/Align/Subset.pm]
#     clean => { FILES=>q[Bio-Align-Subset-*] }
...
LDDLFLAGS = -arch x86_64 -arch i386 -arch ppc -bundle -undefined dynamic_lookup 
...
MAKEMAKER   = /sw/lib/perl5/5.10.0/ExtUtils/MakeMaker.pm
MM_VERSION  = 6.62
MM_REVISION = 66200
...
MAKE = make
FULLEXT = Bio/Align/Subset
BASEEXT = Subset
PARENT_NAME = Bio::Align
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Bio/Align/Subset.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/Bio/Align
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/Bio/Align
...
TO_INST_PM = lib/Bio/Align/Subset.pm

PM_TO_BLIB = lib/Bio/Align/Subset.pm \
...
DISTNAME = Bio-Align-Subset
DISTVNAME = Bio-Align-Subset-1.26
...
manifypods : pure_all  \
	lib/Bio/Align/Subset.pm
	$(NOECHO) $(POD2MAN) --section=3 --perm_rw=$(PERM_RW) \
	  lib/Bio/Align/Subset.pm $(INST_MAN3DIR)/Bio::Align::Subset.$(MAN3EXT) 
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'abstract: '\''A BioPerl module to generate new alignments as subset from larger alignments'\''' >> META_new.yml
	$(NOECHO) $(ECHO) 'author:' >> META_new.yml
	$(NOECHO) $(ECHO) '  - '\''Hector Valverde and Juan Carlos Aledo <hvalverdeuma.es>'\''' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'name: Bio-Align-Subset' >> META_new.yml
	...
	$(NOECHO) $(ECHO) 'version: 1.26' >> META_new.yml
	-$(NOECHO) $(MV) META_new.yml $(DISTVNAME)/META.yml
	$(NOECHO) $(ECHO) Generating META.json
	$(NOECHO) $(ECHO) '{' > META_new.json
	$(NOECHO) $(ECHO) '   "abstract" : "A BioPerl module to generate new alignments as subset from larger alignments",' >> META_new.json
	$(NOECHO) $(ECHO) '   "author" : [' >> META_new.json
	$(NOECHO) $(ECHO) '      "Hector Valverde and Juan Carlos Aledo <hvalverdeuma.es>"' >> META_new.json
	$(NOECHO) $(ECHO) '   ],' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "Bio-Align-Subset",' >> META_new.json
	...
	$(NOECHO) $(ECHO) '   "version" : "1.26"' >> META_new.json
	$(NOECHO) $(ECHO) '}' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1.26">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>A BioPerl module to generate new alignments as subset from larger alignments</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Hector Valverde and Juan Carlos Aledo &lt;hvalverdeuma.es&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  lib/Bio/Align/Subset.pm blib/lib/Bio/Align/Subset.pm 
